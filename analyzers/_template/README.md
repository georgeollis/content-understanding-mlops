...
| `environments.json` | Optional per-family override file. Add only when this family needs environment settings different from repo-root `environments.json` (most commonly a different `labeledDataContainerUrl` per environment for training data). |
| `manifest.dev.json` | Replace `<family>` and the `description` placeholder; leave `promotions`/`current` as-is — `promote-analyzer.ps1` fills those in on first promotion. Copy it again as `manifest.<env>.json` for each additional environment. |
| `golden/` | Add at least one `<name>.<ext>` (any supported document/image format - PDF, DOCX, XLSX, PPTX, PNG, and more; see [`scripts/lib/GoldenDocs.ps1`](../../scripts/lib/GoldenDocs.ps1) for the full list). For `<name>.expected.json`: either hand-write it (fields matching `analyzer.json`'s `fieldSchema`), or promote first and use [`bootstrap-golden.ps1`](../../scripts/bootstrap-golden.ps1) to draft it from the deployed analyzer's own output, then review/correct it. Delete this folder's `.gitkeep`. |
